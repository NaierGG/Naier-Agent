...
4. 큐 배치 크기를 조절하고 싶으면 `WORKFLOW_QUEUE_BATCH_SIZE`를 변경합니다.

큐가 활성화되면:

- 수동 실행은 즉시 실행되지 않고 큐에 등록됩니다.
- 웹훅 실행도 큐에 등록됩니다.
- 스케줄 실행은 먼저 큐에 쌓이고, 같은 크론 사이클에서 워커가 처리합니다.
...
- 큐 적재: [lib/workflow-engine/queue.ts](C:/Users/KANG%20HEE/OneDrive/%EC%BD%94%EB%94%A9/AI%20Agent/lib/workflow-engine/queue.ts)
- 크론 사이클: [lib/workflow-engine/run-queue-cycle.ts](C:/Users/KANG%20HEE/OneDrive/%EC%BD%94%EB%94%A9/AI%20Agent/lib/workflow-engine/run-queue-cycle.ts)
- 수동 처리 엔드포인트: [app/api/queue/process/route.ts](C:/Users/KANG%20HEE/OneDrive/%EC%BD%94%EB%94%A9/AI%20Agent/app/api/queue/process/route.ts)

직접 큐를 처리하고 싶다면:

```bash
curl -X POST "https://your-site.netlify.app/api/queue/process" \
